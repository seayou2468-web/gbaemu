  ...
  switch (swi_imm & 0xFFu) {
    ...
    case 0x0Eu: { // BgAffineSet
       ...
       for (uint32_t i=0; i<count; ++i) {
         ...
         int32_t bg_x = (int32_t)Read32(src+8), bg_y = (int32_t)Read32(src+12);
         int16_t sx = (int16_t)Read16(src+16), sy = (int16_t)Read16(src+18);
         uint16_t angle = Read16(src+20);
         int16_t s = GbaSin(angle), c = GbaCos(angle);
         int16_t pa = (int16_t)((c * sx) >> 14), pb = (int16_t)((-s * sx) >> 14);
         int16_t pc = (int16_t)((s * sy) >> 14), pd = (int16_t)((c * sy) >> 14);
         Write16(dst, pa); Write16(dst+2, pb); Write16(dst+4, pc); Write16(dst+6, pd);
         int32_t rx = bg_x - (int32_t)((int64_t)pa * scr_x + (int64_t)pb * scr_y) / 256;
         int32_t ry = bg_y - (int32_t)((int64_t)pc * scr_x + (int64_t)pd * scr_y) / 256;
    ...
    case 0x10u: { // BitUnPack
      ...
      for (int i=0; i<len; ++i) {
        ...
        if (dst_w == 1) Write8(dst + i/8, (Read8(dst + i/8) & ~(1 << (i%8))) | ((val & 1) << (i%8)));
        else if (dst_w == 2) Write8(dst + i/4, (Read8(dst + i/4) & ~(3 << ((i%4)*2))) | ((val & 3) << ((i%4)*2)));
        else if (dst_w == 4) Write8(dst + i/2, (Read8(dst + i/2) & ~(0xF << ((i%2)*4))) | ((val & 0xF) << ((i%2)*4)));
