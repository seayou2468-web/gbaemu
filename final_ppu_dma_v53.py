...
# Refine StepPpu for more accurate DMA trigger cycle
# Real GBA triggers DMA at the start of HBlank/VBlank.
# We ensure dispstat is updated and WriteIO is done BEFORE StepDma().
...
content = re.sub(r"void GBACore::StepPpu\(uint32_t cycles\) \{.*?^\}", step_ppu_body, content, flags=re.DOTALL | re.MULTILINE)
