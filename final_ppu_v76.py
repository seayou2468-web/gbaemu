...
# Current StepPpu might be advancing ppu_cycle_accum_ too much.
# Let's ensure it's tightly synchronized with the CPU cycle budget.

step_ppu_robust = """void GBACore::StepPpu(uint32_t cycles) {
  ...
  while (remaining > 0) {
    ...
    const uint32_t to_event = (ppu_cycle_accum_ < next_event) ? (next_event - ppu_cycle_accum_) : 1u;
    const uint32_t step = std::min(remaining, to_event);
...
content = re.sub(r"void GBACore::StepPpu\(uint32_t cycles\) \{.*?^\}", step_ppu_robust, content, flags=re.DOTALL | re.MULTILINE)
