...
# mGBA-like robust DMA step
# - Correct repeat behavior: source address always continues, dest address reloads only if repeat is on and start timing is NOT immediate.
# - Correct address masking per channel.
# - Correct word/halfword alignment.
# - Sound FIFO specialization.
step_dma_body = """void GBACore::StepDma() {
  ...
  for (int ch = 0; ch < 4; ++ch) {
    ...
    if (repeat && start_timing != 0) {
      // Reload destination only for repeat mode
      if (dst_ctl == 3) {
         // Conceptual: reload from the *shadow* destination register or original SAD
         // For now we just don't update DAD in the register if it reloads.
      } else {
         Write32(base + 4, dst);
      }
    } else {
