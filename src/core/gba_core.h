...
class GBACore {
 ...
 private:
  ...
  // Main entry: renders scanline y into frame_buffer_[y*240..(y+1)*240-1]
  void DrawScanline(int y);

  // Layer renderers – fill ppu_bg_[bg] / ppu_obj_ + ppu_obj_attr_
  // RGB555 pixels; 0x8000 = transparent sentinel
  void DrawBgText(int bg, int y);      // Modes 0/1/2 – text BGs
  void DrawBgAffine(int bg, int y);    // Modes 1/2   – affine BGs (BG2/3)
  void DrawBgBitmap3(int y);           // Mode 3: 240×160 15bpp
  void DrawBgBitmap4(int y);           // Mode 4: 240×160 8bpp paletted
  void DrawBgBitmap5(int y);           // Mode 5: 160×128 15bpp
  void DrawObjects(int y);             // All OBJ sprites

  // Window mask + compositing
  void BuildWinMask(int y);
  void CompositeScanline(int y);

  // Affine reference-point latch (called at VBlank start)
  void LatchAffineRefs();

  // Fallback debug frame (no ROM or unknown mode)
  ...
  void ScheduleDmaStart(int ch, uint16_t cnt_h,
                        uint32_t delay_cycles_override = 0xFFFFFFFFu);
  bool     IsDmaAddressValid(int ch, uint32_t src, uint32_t dst, bool fifo) const;
  ...
  void EnterException(uint32_t vector_addr, uint32_t new_mode,
  ...
  uint32_t ApplyShift(uint32_t value, uint32_t shift_type,
                      uint32_t shift_amount, bool* carry_out) const;
  bool     GetFlagC() const;
  ...
  enum class BackupType : uint8_t {
    kUnknown = 0, kSRAM = 1, kFlash64K = 2, kFlash128K = 3, kEEPROM = 4,
  };
  ...
  // ── PPU state ─────────────────────────────────────────────────────────────
  RomInfo              rom_info_{};
  std::vector<uint32_t> frame_buffer_;  // ARGB8888, 240×160

  // Scanline pixel buffers (RGB555; 0x8000 = transparent)
  std::array<uint16_t, 240> ppu_bg_[4]{};    // BG layer pixels
  std::array<uint16_t, 240> ppu_obj_{};      // OBJ pixels
  // OBJ per-pixel metadata:
  //   bits[1:0] = priority (0–3)
  //   bit[2]    = semi-transparent mode
  //   bit[3]    = OBJ-window mode
  std::array<uint8_t, 240>  ppu_obj_attr_{};
  // OBJWIN: true where an OBJWIN-mode sprite has a non-transparent pixel
  std::array<bool, 240>     ppu_objwin_{};
  // Window-enable mask per pixel:
  //   bit0=BG0, bit1=BG1, bit2=BG2, bit3=BG3, bit4=OBJ, bit5=ColorEffect
  std::array<uint8_t, 240>  ppu_win_mask_{};

  uint32_t ppu_cycle_accum_ = 0;

  // Affine reference points tracked per scanline (GBA hardware behaviour)
  int32_t ppu_bg2_refx_ = 0;
  int32_t ppu_bg2_refy_ = 0;
  int32_t ppu_bg3_refx_ = 0;
  int32_t ppu_bg3_refy_ = 0;
  bool    ppu_affine_latched_ = false;
  ...
  uint8_t  apu_env_ch1_ = 0, apu_env_ch2_ = 0, apu_env_ch4_ = 0;
  uint8_t  apu_env_timer_ch1_ = 0, apu_env_timer_ch2_ = 0, apu_env_timer_ch4_ = 0;
  uint8_t  apu_len_ch1_ = 0, apu_len_ch2_ = 0, apu_len_ch4_ = 0;
  uint16_t apu_len_ch3_         = 0;
  uint16_t apu_ch1_sweep_freq_  = 0;
  uint8_t  apu_ch1_sweep_timer_ = 0;
  bool apu_ch1_sweep_enabled_ = false;
  bool apu_ch1_active_ = false, apu_ch2_active_ = false;
  bool apu_ch3_active_ = false, apu_ch4_active_ = false;
  bool apu_prev_trig_ch1_ = false, apu_prev_trig_ch2_ = false;
  bool apu_prev_trig_ch3_ = false, apu_prev_trig_ch4_ = false;

  // ── Timers ───────────────────────────────────────────────────────────────
  struct TimerState {
    uint16_t reload = 0, control = 0, counter = 0;
    uint32_t prescaler_accum = 0;
  };
  std::array<TimerState, 4> timers_{};

  // Legacy internal affine refs (kept for backward compat with timing_dma.mm)
  ...
  enum class SioMode : uint8_t {
    kNormal8=0, kNormal32=1, kMulti=2, kUart=3, kGpio=4, kJoybus=5,
  };
  struct SioState {
  ...
  struct DmaState {
    ...
    bool pending=false, in_progress=false;
    uint32_t wait_cycles=0, last_value=0;
    bool seq_access=false;
    uint32_t prev_src_addr=0, prev_dst_addr=0;
  };
  ...
  mutable uint8_t eeprom_addr_bits_ = 0;
