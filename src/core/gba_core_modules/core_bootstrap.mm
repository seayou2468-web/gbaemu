...
bool GBACore::LoadBIOS(const std::vector<uint8_t>& bios, std::string* error) {
  if (bios.size() != 16384) { if (error) *error = "Invalid BIOS size"; return false; }
  std::copy(bios.begin(), bios.end(), bios_.begin());
  bios_loaded_ = true; bios_is_builtin_ = false; return true;
}

void GBACore::LoadBuiltInBIOS() { bios_is_builtin_ = true; bios_loaded_ = true; }

bool GBACore::LoadROM(const std::vector<uint8_t>& rom, std::string* error) {
  if (rom.size() < 0x100) { if (error) *error = "ROM too small"; return false; }
  rom_ = rom;
  ...
  Reset(); loaded_ = true; return true;
}

uint8_t GBACore::ComputeComplementCheck() const {
  uint8_t res = 0;
  for (size_t i = 0xA0; i <= 0xBC; ++i) res = res - rom_[i];
  return (res - 0x19);
}

bool GBACore::ValidateNintendoLogo() const { return true; }
...
void GBACore::RunCycles(uint32_t cycles) {
  ...
  while (cycles_to_run > 0) {
    ...
    StepPpu(ran); StepTimers(ran); StepApu(ran); StepSio(ran);
    executed_cycles_ += ran; if (ran >= cycles_to_run) break; cycles_to_run -= ran;
  }
}

void GBACore::StepFrame() { RunCycles(kCyclesPerFrame); frame_count_++; }

void GBACore::SetKeys(uint16_t keys_pressed_mask) {
  previous_keys_mask_ = keys_pressed_mask_; keys_pressed_mask_ = keys_pressed_mask; SyncKeyInputRegister();
}
