...
void GBACore::Reset() {
  ...
  for (auto& line : bg_hofs_line_) line.fill(0);
  for (auto& line : bg_vofs_line_) line.fill(0);
  for (auto& line : bg_cnt_line_) line.fill(0);
  for (auto& p : bg2_affine_line_) p = {};
  for (auto& p : bg3_affine_line_) p = {};
