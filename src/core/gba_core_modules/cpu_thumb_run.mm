...
void GBACore::ExecuteThumbInstruction(uint16_t opcode) {
  ...
  if ((opcode & 0xFC00u) == 0x4000u) {
    ...
    switch (op) {
      ...
      case 0x2: {
        bool carry = GetFlagC();
        cpu_.regs[rd] = ThumbApplyRegisterShift(d, 0, s & 0xFFu, GetFlagC(), &carry);
        SetNZFlags(cpu_.regs[rd]);
        SetFlagC(carry);
        break;
      }
      case 0x3: {
        bool carry = GetFlagC();
        cpu_.regs[rd] = ThumbApplyRegisterShift(d, 1, s & 0xFFu, GetFlagC(), &carry);
        SetNZFlags(cpu_.regs[rd]);
        SetFlagC(carry);
        break;
      }
      case 0x4: {
        bool carry = GetFlagC();
        cpu_.regs[rd] = ThumbApplyRegisterShift(d, 2, s & 0xFFu, GetFlagC(), &carry);
        SetNZFlags(cpu_.regs[rd]);
        SetFlagC(carry);
        break;
      }
