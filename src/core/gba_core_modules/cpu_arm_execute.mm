  ...
  if ((opcode & 0x0E000000u) == 0x08000000u) {
    ...
    if (rlist == 0) {
      ...
      if (load) {
        ...
        if (writeback) cpu_.regs[rn] = up ? (cpu_.regs[rn] + 0x40u) : (cpu_.regs[rn] - 0x40u);
        return;
      } else {
    ...
    auto read_transfer_reg = [&](uint32_t r) -> uint32_t {
      ...
      if (r >= 8 && r <= 12) {
        return (GetCpuMode() == 0x11u) ? cpu_.banked_usr_r8_r12[r - 8] : cpu_.regs[r];
      }
    ...
    auto write_transfer_reg = [&](uint32_t r, uint32_t value) {
      ...
      if (r >= 8 && r <= 12) {
        if (GetCpuMode() == 0x11u) cpu_.banked_usr_r8_r12[r - 8] = value;
