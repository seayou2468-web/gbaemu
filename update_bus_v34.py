...
# 1. Consolidated Read Logic
# We'll implement ReadBus(addr, size) as a conceptual helper,
# but for performance we'll keep the specialized methods and use common internal logic.
...
read16 = """uint16_t GBACore::Read16(uint32_t addr) const {
  if (addr & 1u) {
     // Unaligned read16 behaves like sequential 8-bit reads
...
# Correct PPU write rules and aligned/unaligned logic
write32 = """void GBACore::Write32(uint32_t addr, uint32_t value) {
  ...
  Write16(addr & ~3u, static_cast<uint16_t>(value));
  Write16((addr & ~3u) + 2u, static_cast<uint16_t>(value >> 16));
}"""
...
content = replace_func(full_content, "ReadBus32", readbus32)
