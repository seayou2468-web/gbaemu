...
int16_t GbaSin(uint16_t angle) {
  // GBA sine table uses 16384 as 1.0 (1.14 fixed point)
  ...
  switch (swi_imm & 0xFFu) {
    ...
    case 0x0Eu: { // BgAffineSet
       ...
       for (uint32_t i=0; i<count; ++i) {
         int32_t cx = (int32_t)Read32(src);
         int32_t cy = (int32_t)Read32(src+4);
         int16_t dx = (int16_t)Read16(src+8);
         int16_t dy = (int16_t)Read16(src+10);
         int16_t sx = (int16_t)Read16(src+12);
         int16_t sy = (int16_t)Read16(src+14);
         uint16_t theta = Read16(src+16);
         int16_t s = GbaSin(theta), c = GbaCos(theta);
         int16_t pa = (int16_t)((c * sx) >> 14);
         int16_t pb = (int16_t)((-s * sx) >> 14);
         int16_t pc = (int16_t)((s * sy) >> 14);
         int16_t pd = (int16_t)((c * sy) >> 14);
         Write16(dst, pa); Write16(dst+2, pb); Write16(dst+4, pc); Write16(dst+6, pd);
         int32_t rx = cx - (int32_t)(((int64_t)pa * dx + (int64_t)pb * dy) >> 8);
         int32_t ry = cy - (int32_t)(((int64_t)pc * dx + (int64_t)pd * dy) >> 8);
